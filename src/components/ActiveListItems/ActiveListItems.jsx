...
export const ActiveListItems = () => {
  ...
  useEffect(() => {
    const newFilteredList = uniqueChecklists.filter(it =>
      filtered.includes(it?.pointOfRouter)
    );
  ...
  return (
    <ActiveListItemsSection>
      <ActiveListItemsContainer>
        <FilterComponent filtered={filtered} setFiltered={setFiltered} />
        <ControlContainer>
          <ControlPanelContainer>
            <TotalItems>
              <span style={{ whiteSpace: 'nowrap' }}>
                Текущих чек-листов:&nbsp;{filteredList?.length}
              </span>
            </TotalItems>
            <BtnContainer>
              ...
              item =>
                ...
                item?.identifier !== '' && (
                  ...
                  >
                    ...
                    <ItemStatistic>
                      {(() => {
                        const caseConfig = POINT_OF_CASES[item?.pointOfRouter];
                        if (!caseConfig) return null;

                        const levelsCount = Object.keys(caseConfig).length;

                        return Array.from({ length: levelsCount }, (_, idx) => {
                          const level = `level${idx + 1}`;
                          const { itemCircleProps, itemLineProps } =
                            getItemProps(
                              level,
                              item?.pointOfRouter,
                              item,
